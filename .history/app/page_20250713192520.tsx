...
export default function HomePage() {
  ...
  const filtered = data?.filter((product) => {
    ...
    const matchPrice =
      ...
      (priceFilter === 'mid' && product.price >= 300000 && product.price <= 800000) ||
  ...
  if (isError) return <p className=" bg text-center py-20 text-red-600">Failed to load products.</p>;
      ...
      >
        <div className="relative z-10">
          ...
          <p className="text-lg text-black dark:text-white max-w-xl mx-auto">
            Explore our curated selection of the latest smartphones tailored to your needs.
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {filtered?.map((product) => (
          ...
          >
            ...
            />
            <h2 className="font-semibold text-xl  dark:text-gray-500">{product.name}</h2>
            <p className="text-gray-900 dark:text-gray-300">{product.storage}</p>
  ...
  <h2 className="text-2xl font-bold mb-4 text-black dark:text-white">Visit Our Store</h2>
